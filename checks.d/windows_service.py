...
class WindowsService(WinWMICheck):
    ...
    def check(self, instance):
        ...
        self._process_services(wmi_sampler, services, [u'host:{0}'.format(host)])
    ...
    def _process_services(self, wmi_sampler, services, tags):
        ...
        for wmi_obj in wmi_sampler:
            ...
            self.service_check("windows_service.state", status, tags=tags + ['service:%s' % service])
            expected_services.remove(service)

        for service in expected_services:
            self.service_check("windows_service.state", AgentCheck.CRITICAL, tags=['service:%s' % service])
